<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <form action="{{ route('cariler.store') }}" method="post" enctype="multipart/form-data">
                @csrf

                <div class="flex flex-row gap-5 bg-white p-5 rounded-lg flex-wrap">
                    <div>
                        <x-input-label :value="__('Cari Adı')"/>
                        <x-text-input class="w-32" name="adi" placeholder="Cari Adı" value="{{ old('adi') }}"/>
                    </div>
                    <div>
                        <x-input-label :value="__('Cari Soyadı')"/>
                        <x-text-input class="w-32" name="soyadi" placeholder="Cari Soyadı" value="{{ old('soyadi') }}"/>
                    </div>
                    <div>
                        <x-input-label :value="__('Email')"/>
                        <x-text-input type="email" name="email" placeholder="Email" value="{{ old('email') }}"/>
                    </div>
                    <div>
                        <x-input-label :value="__('Kimlik No')"/>
                        <x-text-input class="w-40" type="number" name="kimlikno" placeholder="Kimlik No" value="{{ old('kimlikno') }}"/>
                    </div>
                    <div>
                        <x-input-label :value="__('Vergi No')"/>
                        <x-text-input class="w-40" type="number" name="vergino" placeholder="Vergi No" value="{{ old('vergino') }}"/>
                    </div>
                    <div>
                        <x-input-label :value="__('Telefon')"/>
                        <x-text-input class="w-40" type="number" name="telefon" placeholder="Telefon" value="{{ old('telefon') }}"/>
                    </div>
                    <div>
                        <x-input-label :value="__('Cari Tipi')"/>
                        <select data-te-select-init name="tip" class="rounded-lg border-gray-300">
                            <option value="1">{{__('Alıcı')}}</option>
                            <option value="2">{{__('Satıcı')}}</option>
                            <option value="3">{{__('Personel')}}</option>
                        </select>
                    </div>
                    <div>
